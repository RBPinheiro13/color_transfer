function [img_fusion] = distance2_2(img_fusion,Q_inv,img_target,A_mean,numlabels_target,labels_target,r_2,C_Br,C_Bg,C_Bb,SP_Fusion,I,J)
...
for co1 = 1:size(I,1)
    ...
    for co2 = 1:size(J,1)
		...
		img_fusion(pos,pos2,1) = sum(bsxfun(@times,w',C_Br))/sum(w);
        img_fusion(pos,pos2,2) = sum(bsxfun(@times,w',C_Bg))/sum(w);
        img_fusion(pos,pos2,3) = sum(bsxfun(@times,w',C_Bb))/sum(w);
