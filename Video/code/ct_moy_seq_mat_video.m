function [ img,corresp,R_t,G_t,B_t] = ct_moy_seq_mat_video(R_s,G_s,B_s, numlabels_source,R_t,G_t,B_t, labels_target, numlabels_target,sp_limit,n_iter)
...
V_fin = 9*(255^2);
V_comp = zeros(numlabels_target,numlabels_source);
Check = zeros(1,numlabels_source);
changes = 0;
corresp = zeros(numlabels_target,3);

[i,j,k] = size(labels_target);
img = zeros(i,j,3,k);
...
for pos3 = 1:k
    for pos = 1:i
        for pos2 = 1:j
            ...
            x = labels_target(pos,pos2,pos3);
            
            img(pos,pos2,1,pos3) = R_t(x);
            img(pos,pos2,2,pos3) = G_t(x);
            img(pos,pos2,3,pos3) = B_t(x);
        end
    end
