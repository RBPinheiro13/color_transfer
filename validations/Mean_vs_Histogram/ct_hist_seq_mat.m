function [ img,corresp ] = ct_hist_seq_mat(R_s,G_s,B_s,h_s, numlabels_source,R_t,G_t,B_t,h_t, labels_target, numlabels_target,sp_limit,n_iter)
%Function to do the correspondance between superpixels in images.
%Where RGB are divided in 3 columns for each image, source and target. And
%h is the histogram (normalized and cumulated) of each image.
%The other parameters are provided by the division in superpixel after
%SLIC.
%
%Example:
%[ img,corresp ] = ct_hist_seq_mat(R_s,G_s,B_s,h_s, numlabels_source,R_t,G_t,B_t,h_t, labels_target, numlabels_target,sp_limit,n_iter)
%
%Where:
% - R_s,G_s,B_s = RGB of the SOURCE image
% - h_s = histogram normalized and cumulated of the SOURCE image
% - numlabels_source = number of labels in the superpixel division of SOURCE image
% - R_t,G_t,B_t = RGB of the TARGET image
% - h_t = histogram normalized and cumulated of the TARGET image
